...
struct AssetRepositoryAIChatView: View {
    ...
    private func makeSystemPrompt(modelName: String) -> String {
        ...
        - Before calling a 3D generation tool, the user must have provided the desired asset name. If no name is provided, ask exactly one concise follow-up question for the name, then pass it as asset_name.
