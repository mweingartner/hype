...
/// as a normal provider preference and sends text/tool/schema calls through an
/// OpenAI-compatible chat-completions inference client, while model discovery
/// uses llama-swap's `GET /v1/models` endpoint.
public actor LlamaSwapClient: HypeAIClient {
    ...
    public nonisolated func chatStream(messages: [OllamaMessage], tools: [OllamaTool]) -> AsyncStream<String> {
        openAIInferenceClient().chatStream(messages: messages, tools: tools)
    }

    public func availableModels() async throws -> [String] {
        let url = Self.endpoint("v1/models", baseURL: baseURL)
    ...
    public func generate(prompt: String, model overrideModel: String? = nil, system: String? = nil) async throws -> String {
        try await wrap {
            try await openAIInferenceClient().generate(prompt: prompt, model: overrideModel, system: system)
        }
    }
    ...
    public func chat(
        ...
        try await wrap {
            try await openAIInferenceClient().chat(messages: messages, tools: tools, format: format)
        }
    }
    ...
    public func structuredChat<Response: Decodable & Sendable>(
        ...
        try await wrap {
            try await openAIInferenceClient().structuredChat(messages: messages, tools: tools, format: format)
        }
    }
    ...
    private nonisolated func openAIInferenceClient() -> OpenAIChatCompletionsClient {
