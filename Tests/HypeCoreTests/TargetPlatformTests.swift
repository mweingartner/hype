...
struct TargetPlatformTests {
    ...
    func runtimePackageBuilderEmbedsSelfContainedStack() throws {
        ...
        #expect(runtimeCorePackage.contains(".package(path: \"Vendor/AudioKit\")"))
        #expect(!runtimeCorePackage.contains("github.com/AudioKit/AudioKit.git"))
        #expect(runtimeCorePackage.contains("exclude: [\"MCP\"]"))
        #expect(FileManager.default.fileExists(atPath: shellDir.appendingPathComponent("HypeSource/Sources/HypeCore").path))
        #expect(!FileManager.default.fileExists(atPath: shellDir.appendingPathComponent("HypeSource/Sources/HypeCore/MCP").path))
        #expect(FileManager.default.fileExists(atPath: shellDir.appendingPathComponent("HypeSource/Vendor/AudioKit/Package.swift").path))
        #expect(!FileManager.default.fileExists(atPath: shellDir.appendingPathComponent("HypeSource/Package.resolved").path))
