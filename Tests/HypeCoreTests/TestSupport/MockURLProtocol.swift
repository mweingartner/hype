...
/// Dedicated mock URLProtocol for OpenAIResponsesClientTests.
final class MockURLProtocolOpenAIResponses: URLProtocol {
    nonisolated(unsafe) static var requestHandler: ((URLRequest) throws -> (HTTPURLResponse, Data))?
    override class func canInit(with request: URLRequest) -> Bool { true }
    override class func canonicalRequest(for request: URLRequest) -> URLRequest { request }
    override func startLoading() {
        guard let handler = MockURLProtocolOpenAIResponses.requestHandler else {
            client?.urlProtocol(self, didFailWithError:
                NSError(domain: "MockURLProtocolOpenAIResponses", code: -1,
                        userInfo: [NSLocalizedDescriptionKey: "No handler set"]))
            return
        }
        do {
