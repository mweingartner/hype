...
- Deferred (documented in design-mock.md §7, not addressed here): an icon-picker UI for `iconId`; a real sequencer/mixer track editor for `musicTrackData` (the Inspector now shows a read-only track count); a `pdfAssetRef` AI binding tool (parity with scene3D's binding); the target-not-found SET fallback (`Interpreter.swift:1603-1605`, unrelated residual typo-swallower) and GET of a fully unknown property name (stays `""`, unchanged — only SET became strict).
...
**Polymorphic dispatch.** A small set of bare, ambiguous words — `value`, `min`, `max`, `step`, `loop`, `volume`, `autoplay`, `playRate`, `currentTime`, `duration`, `tint`, `prompt`, `total`, `items`, `decimals`, `color`, `contents`, `style`, `background` — resolve per the target part's type before the flat registry lookup ever runs. For example, bare `min`/`max` map to `controlMin`/`controlMax` on stepper/slider, `gaugeMin`/`gaugeMax` on gauge, `minDate`/`maxDate` on calendar, and (max only — min is pinned to 0) `progressTotal` on progressView; any other part type errors naming the property and listing the types it does apply to. Long, type-prefixed names (`gaugeMax`, `videoLoop`, …) always work directly regardless of the bare-word table.
...
```
returns `"(masked)"` rather than the field's real text. `set_part_property(part_name:"password", property:"value", value:"...")` still writes normally (masking applies to reads); `get_part_property(part_name:"password", property:"textContent")` and `...property:"searchText"` on a search-style secure field are masked the same way, on both surfaces.
