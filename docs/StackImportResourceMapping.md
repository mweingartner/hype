...
- For multi-stack project imports, stack-library entries marked
  `contentStack=true` are content/library stacks. Their imported repository
  assets are copied into the other generated documents with
  `shared_from_content_stack` provenance metadata so each `.hype` remains
  self-contained while classic media/resource lookups can find shared Myst-style
  content by name or resource id.
...
| `PICT` | PNG payload from the narrow PICT adapter plus native resource record | `AssetKind.imageTexture` | Imported assets carry resource type/id, classic-name, and lookup-key metadata. `HTAddPict`/`HTChangePict` can create current-card compatibility image parts from those assets, crop decoded images for classic `srcRect` arguments, record recognized transfer modes, pair `HTSavePict` clipboard saves with transient overlay removal, and let `HTRemove` clear compatibility-created visual parts; full pixel compositing and palette behavior remain future work. |
...
| `FONT`, `NFNT`, `finf`, `sfnt`, `FWID` | Font metadata or native font bytes when supported | Legacy metadata; possible embedded font asset only after a font policy exists | Does not instantiate a part. StackImport package font tables are recorded in diagnostics, and imported text degrades to deterministic available macOS fallback fonts until font embedding is designed. |
| `clut`, `CTBL`, `actb`, `cctb`, `dctb`, `fctb`, `wctb`, `pltt`, `PLTE` | JSON color-table or palette payloads, optionally preview PNGs | Logical palette asset using `Asset.metadata` plus related preview `Asset.files`; possible theme/palette candidate | Does not instantiate a part. `HTUDefPal` resolves these imported assets by `resource_id`, parses StackImport JSON entries into normalized `#RRGGBB` color globals, records script-visible compatibility state, and lets QuickDraw `xLine` render active palette color indexes into the current paint layer. Future theme import may create stack theme tokens or swatches. |
