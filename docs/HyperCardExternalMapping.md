...
| `HTRemove` | Emulated | Clears compatibility-created PICT/icon/video parts and records the removed count. | Myst visual/palette runtime slice. Used by Myst stack teardown after HyperTint setup. |
...
| `HyperTint` | Emulated | Records HyperTint timing/delay/options as runtime-only compatibility state and leaves rendering unchanged. | Myst visual/palette runtime slice. Future work should map HyperTint/AddColor resources to card/background paint layers or overlays. |
...
| `xClip` | Emulated | Parses a classic rect, records runtime-only QuickDraw clip-rect intent, and returns the normalized rect in `it`/`the result`. | Myst visual/palette runtime slice. Future work should apply the clip to a paint-layer/overlay renderer once transient QuickDraw drawing is modeled. |
...
| `HTTB1TS` | Emulated | Parses destination/source classic rects, transfer-mode options, and VBL/noVBL options for HyperTint temp-buffer-to-screen tile copies; records runtime-only tile-copy intent and count. | Myst visual/palette runtime slice. Future work should composite the HyperTint temp buffer into the real card/overlay paint layer for dropped-page animation. |
| `Drop` | Emulated | Records Myst's dropped page visual intent (`BluePage`, `RedPage`, or `AtrusPage`) plus the prior `ALL_Page` value as runtime-only compatibility state and leaves page inventory mutation to the calling script. | Myst page runtime slice. Future work should render the full HyperTint temp-buffer page-drop animation rather than only preserving script-visible state. |
| `Picture` | Emulated | Resolves an imported picture resource by classic name/metadata and creates an image-backed compatibility window part on the current card. Script-visible window existence, `rect`, `loc`, `scroll`, `dithering`, `show window`, and `close window <name>` state are modeled through runtime globals and the image part. Missing assets set a nonempty `the result` diagnostic. | Myst visual/palette runtime slice. Future work should implement exact scroll/crop rendering and classic floating-window compositing. |
