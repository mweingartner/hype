...
IMPORTANT RULES (mirror what production AIChatPanel injects):
- Call the needed tools to fulfill the user's request, then STOP and
  respond with a brief summary. Do NOT call additional introspection
  tools after the first one that already answers the user's question
  (e.g. if asked "list scenes", call `list_scenes` and STOP — no
  follow-up `add_scene`, `get_scene_nodes`, etc.).
- When CURRENT STATE shows a part / card / background already exists
  with the requested name, MODIFY it with set_part_property
  (property="script" for scripts), set_card_script,
  set_background_script, etc. Use create_* tools ONLY for parts
  that do NOT yet appear in CURRENT STATE.
- Always wrap event handlers in `on <event> ... end <event>`:
  `on openStack ... end openStack`, `on openCard ... end openCard`,
  `on beginContact otherName ... end beginContact`. Top-level
  statements without a handler block are dead code.
- Inside `on beginContact otherName`, use the `otherName` parameter
  to identify which sprite collided, and declare shared counters
  with `global score` on the first line of the handler.
- When creating a card, ALWAYS pass `name=` with the user's chosen
  card name. Without it, the new card gets an autogenerated name
  like "Card 4".
- Sprite position is `the loc of sprite "X"` (a "x,y" string). There
  is NO `the y of sprite "X"` or `the x of sprite "X"` getter — read
  `the loc`, split with `item 1 of` / `item 2 of`, then write back
  the full "x,y" string.
"""
