...
socket under `<discovery>/<pid>.sock` plus a JSON descriptor for discovery. Discovery
uses `HYPE_DEBUG_SOCKET_DIR` env var, then `.hype/debug/` relative to the
repo, then `~/Library/Application Support/Hype/debug/`. That
