...
defaulting to `c,8`. `createMenu` records imported menu construction as
runtime-only metadata, and `soundTime` records classic QuickTime/audio timing
segment arguments. `HTAddPict`, `HTChangePict`, `HTSavePict`, `HTRemove`,
...
breakpoints halt at handler entry after parameters and implicit event locals
are bound; the debugger or debug/MCP bridge can inspect that paused frame and
resume the same dispatch. These debugger controls are process/session state
only and are not persisted into `.hype` documents.
...
files, plus source package paths, generated `.hype` package byte counts, and
import durations for live import profiling; these are optional legacy-import
report metadata and do not require a document-version migration.
